...
if not df.empty:
    ...
    with col1:
        if not df_today.empty:
            fig_today = px.pie(df_today, values='Hours', names='Study', 
                               title='Today\'s Study Hours by Subject',
                               hole=0.4)
            fig_today.update_traces(textinfo='label+value')  # Show the actual number of hours
    ...
    with col2:
        if not df_today.empty:
            ...
            st.markdown(f"<h1 style='text-align: center; color: #3E54AC;'>{total_hours_today:.2f} Hours</h1>", unsafe_allow_html=True)

    # Remaining part of your original code...
